...
class FGF_Rules_Tab extends FGF_Settings_Page {
	...
	public function __construct() {
		...
		$this->label = __('Rules', 'buy-x-get-y-promo');
	...
	public function display_rule_configuration_notices() {
		FGF_Settings::notice(__('Please make sure the products you select as free gifts have been published and are currently In-stock.', 'buy-x-get-y-promo'));
	}
	...
	public function render_rules_table() {
		...
		echo '<h1 class="wp-heading-inline">' . esc_html__('Free Gift Rules', 'buy-x-get-y-promo') . '</h1>';
		echo '<a class="page-title-action fgf_add_btn" href="' . esc_url(fgf_get_rule_page_url(array( 'action' => 'new' ))) . '">' . esc_html__('Add New Rule', 'buy-x-get-y-promo') . '</a>';
	...
	private static function get_rule_data_tabs() {
		...
		$tabs = apply_filters(
			...
			'general' => array(
				'label' => __('General', 'buy-x-get-y-promo'),
				'target' => 'fgf_rule_data_general',
				'class' => array(),
				'priority' => 10,
				),
				'restrictions' => array(
				'label' => __('Restrictions', 'buy-x-get-y-promo'),
				'target' => 'fgf_rule_data_restrictions',
				'class' => array(),
				'priority' => 20,
				),
				'criteria' => array(
				'label' => __('Criteria', 'buy-x-get-y-promo'),
				'target' => 'fgf_rule_data_criteria',
				'class' => array(),
				'priority' => 30,
				),
				'filters' => array(
				'label' => __('Filters', 'buy-x-get-y-promo'),
				'target' => 'fgf_rule_data_filters',
				'class' => array(),
				'priority' => 40,
				), 'notices' => array(
				'label' => __('Notice', 'buy-x-get-y-promo'),
	...
	public function save() {
		...
		if (isset($_GET['message'])) {
			...
			switch ($message) {
				case 'created':
					FGF_Settings::add_message(__('New Rule has been created successfuly', 'buy-x-get-y-promo'));
					break;

				case 'updated':
					FGF_Settings::add_message(__('Rule has been updated successfully', 'buy-x-get-y-promo'));
					break;
			}
		}
	...
	public function prepare_rule_data() {
		...
		if ('' == $rule_post_data['fgf_rule_name']) {
			throw new Exception(esc_html__('Rule Name is mandatory', 'buy-x-get-y-promo'));
		}
		...
		switch ($rule_post_data['fgf_rule_type']) {
			case '7':
			case '8':
				if ('3' === $rule_post_data['fgf_subtotal_price_type'] && empty($total_categories)) {
					throw new Exception(esc_html__('Please select a category', 'buy-x-get-y-promo'));
				}

				if (empty($rule_post_data['fgf_subtotal_price'])) {
					throw new Exception(esc_html__('Please provide the value for subtotal price', 'buy-x-get-y-promo'));
				}

				if ('8' === $rule_post_data['fgf_rule_type'] && empty($subtotal_gift_products)) {
					throw new Exception(esc_html__('Please select atleast one product', 'buy-x-get-y-promo'));
					...
					throw new Exception(esc_html__('Please select atleast one category', 'buy-x-get-y-promo'));
				}

				if (empty($rule_post_data['fgf_subtotal_gift_products_qty'])) {
					throw new Exception(esc_html__('Quantity for Selected Free Gift Product(s) cannot be empty', 'buy-x-get-y-promo'));
				}
				...
				if (isset($rule_post_data['fgf_subtotal_repeat_gift']) && '2' === $rule_post_data['fgf_subtotal_repeat_gift_mode'] && empty($rule_post_data['fgf_subtotal_repeat_gift_limit'])) {
					throw new Exception(esc_html__('Please enter the limit', 'buy-x-get-y-promo'));
				}
				break;
			...
			case '4':
				if (empty($apply_coupon)) {
					throw new Exception(esc_html__('Please select a coupon', 'buy-x-get-y-promo'));
				}
			...
			case '5':
				...
				if ('1' == $rule_post_data['fgf_buy_product_type'] && empty($buy_product)) {
					throw new Exception(esc_html__('Please select atleast one Product for buy product', 'buy-x-get-y-promo'));
				} elseif ('2' == $rule_post_data['fgf_buy_product_type'] && empty($buy_categories)) {
					throw new Exception(esc_html__('Please select atleast one Category for buy product', 'buy-x-get-y-promo'));
				}
				...
				if ('5' != $rule_post_data['fgf_rule_type'] && '2' == $rule_post_data['fgf_bogo_gift_type'] && empty($get_products)) {
					throw new Exception(esc_html__('Please select atleast one Product for get product', 'buy-x-get-y-promo'));
				} elseif ('5' == $rule_post_data['fgf_rule_type'] && '2' == $rule_post_data['fgf_get_product_type'] && empty($get_categories)) {
					throw new Exception(esc_html__('Please select atleast one Category for get product', 'buy-x-get-y-promo'));
				...
				if (empty($rule_post_data['fgf_buy_product_count'])) {
					throw new Exception(esc_html__('Buy Product count cannot be empty.', 'buy-x-get-y-promo'));
				}

				//Validate if the get product count does not exist.
				if (empty($rule_post_data['fgf_get_product_count'])) {
					throw new Exception(esc_html__('Get Product count cannot be empty.', 'buy-x-get-y-promo'));
				}
				...
				if (isset($rule_post_data['fgf_bogo_gift_repeat']) && '2' == $rule_post_data['fgf_bogo_gift_repeat_mode'] && empty($rule_post_data['fgf_bogo_gift_repeat_limit'])) {
					throw new Exception(esc_html__('Repeat Limit field cannot be empty.', 'buy-x-get-y-promo'));
				}
		...
		if ($non_purchasable_product) {
			...
			throw new Exception(wp_kses_post(sprintf(__('The selected product(s) %s cannot be set as free gifts. Please make sure the products you select as free gifts have been published and are currently In-stock.', 'buy-x-get-y-promo'), $non_purchasable_product)));
		}
		...
		if ('2' == $rule_post_data['fgf_product_filter_type'] && '4' == $rule_post_data['fgf_applicable_products_type']) {
			if ($rule_post_data['fgf_include_product_count'] && count($include_products) < $rule_post_data['fgf_include_product_count']) {
				throw new Exception(esc_html__('Product Count cannot be more than the number of selected products.', 'buy-x-get-y-promo'));
			} else if (!$rule_post_data['fgf_include_product_count']) {
				throw new Exception(esc_html__('Product Count cannot be empty.', 'buy-x-get-y-promo'));
			}
		}
		...
		if ('5' == $rule_post_data['fgf_product_filter_type'] && '4' == $rule_post_data['fgf_applicable_categories_type'] && !$rule_post_data['fgf_include_category_product_count']) {
			throw new Exception(esc_html__('Minimum Quantity field cannot be empty.', 'buy-x-get-y-promo'));
		}
